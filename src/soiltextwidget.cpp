...
void SoilTextWidget::adjust_content_view_to_cursor()
{
    ...
    //CENTRALIZE
    if(cursor_rect.x < content_view_bound.start_x)
    {
        mov_x = ((cursor_rect.x - char_size.w) - content_view_bound.start_x);
    }
    if(cursor_rect_ex > content_view_bound.end_x)
    {
        mov_x = (cursor_rect_ex - content_view_bound.end_x);
    }

    if(cursor_rect.y < content_view_bound.start_y)
    {
        mov_y = (cursor_rect.y - content_view_bound.start_y);
    }
    if(cursor_rect_ey > content_view_bound.end_y)
    {
        mov_y = (cursor_rect_ey - content_view_bound.end_y);
    }
