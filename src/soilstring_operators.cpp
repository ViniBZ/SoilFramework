...
{
    bool prev_signal_change = signal_change;
    FILE* fo = NULL;
    ...
    signal_change = prev_signal_change;
    fclose(fo);
...
{    
    if(is_valid_relative_cursor_pos(length, pos))
    {
...
// or -1 if not found
int SoilString::search_str(const SoilString& str, int pos, int n, bool match_case, bool match_accent, bool left_to_right) const
{
    //MARKER:check if those should really be the return values for empty strings
    if(length == 0){return -2;}
    if(str.length == 0){return -3;}
    validate_rel_cursor_range(length, &pos, &n);
    //if(!left_to_right){pos -= 1;}
    if(this == &str){return 0;}
    ...
    while(i != end)
    {
        ...
        if(soil_char_are_equal(ch, sub_ch, match_case, match_accent))
        {
            ...
            if(left_to_right){i -= sub_c;}else{i += sub_c;}
            sub_i = sub_start;
            sub_c = 0;
            sub_ch = str.at(sub_i);
        }
...
int SoilString::search_str(const SoilString& str, int pos, bool match_case, bool match_accent, bool left_to_right) const
{
    ...
    return search_str(str,pos,n,match_case,match_accent,left_to_right);
}
// ----------------------------------------------- SEARCH STR
// return the point where the first match was found
// or -1 if not found
...
{
    return search_str(str, pos, match_case, match_accent, true);
}
// ----------------------------------------------- SEARCH STR
int SoilString::search_str(const char* str, int pos, int n, bool match_case, bool match_accent, bool left_to_right) const//return sub_str po
{
    SoilString soil_str(str);
    return search_str(soil_str, pos, n, match_case, match_accent, left_to_right);
}
// ----------------------------------------------- SEARCH STR
int SoilString::search_str(const char* str, int pos, bool match_case, bool match_accent, bool left_to_right) const//return sub_str po
{
    SoilString soil_str(str);
    return search_str(soil_str, pos, match_case, match_accent, left_to_right);
}
// ----------------------------------------------- SEARCH STR
int SoilString::search_str(const char* str, int pos, bool match_case, bool match_accent) const//return sub_str po
{
    SoilString soil_str(str);
    return search_str(soil_str, pos, match_case, match_accent, true);
}
// ----------------------------------------------- SEARCH REPLACE STR
...
int SoilString::search_replace_str(const SoilString& str, const SoilString& rep, int pos, int n, bool match_case, bool match_accent)
{
    ...
    {
        cur_n = n - pos_added;
        res = search_str(str, res, cur_n, match_case, match_accent, true);
        if(res >= 0)
        {
    ...
    if(signal_change && found_n > 0 && str != rep){str_changed();}
...
int SoilString::replace_tabs_with_spaces(int space_n)
{
    ...
    if(signal_change && res > 0){str_changed();}
    return res;
}
