...
typedef enum UI_TYPE{LAYOUT,WIDGET}UI_TYPE;
typedef enum WIDGET_TYPE{LABEL,INPUT,TEXTBLOCK,TEXTFIELD,IMAGE,BUTTON,COLORBUTTON,DROPMENU,LIST,CANVAS,GRAPH,SLIDER,PROGRESSBAR,VIEW,NO_WIDGET}WIDGET_TYPE;
typedef enum LAYOUT_TYPE{VERTICAL,HORIZONTAL}LAYOUT_TYPE;
typedef enum ALIGNMENT_TYPE{CENTER,TOP,BOTTOM,LEFT,RIGHT}ALIGNMENT;
typedef enum LAYOUT_NEST_PROPERTY{ALL_PROPERTIES=0xFF,CONTROL=0x01,FONT=0x02,DEST_PIXMAP=0x04}LAYOUT_NEST_PROPERTY;
...
typedef struct LINEAR_RANGE{
    int s;//start
    int e;//end
}LINEAR_RANGE;


typedef struct CALC_PIX_COLOR
{
    double b;
    double g;
    double r;
...
typedef union PIXDATA
{
    Uint32 pix;
    PIXCOLOR color;
}PIXDATA;

typedef struct PIXDATA_FLAG
{
    PIXDATA pixdata;
    bool flag;
}PIXDATA_FLAG;
...
typedef struct SCROLL_DATA_STRUCT{
    bool vertical;
    bool horizontal;
...
typedef struct UI_SIZES_STRUCT{
    SIZE size;
    SIZE min_size;
    SIZE max_size;
}UI_SIZES;
...
typedef struct SCROLL_ARG{
    bool scroll_set;
    double scroll_ratio;
    int scroll_pos;
}SCROLL_ARG;

typedef struct CONTENT_VIEW_ARG{
    POINT content_view_pos;

}CONTENT_VIEW_ARG;

typedef struct MOUSE_ARG{
    Uint8 button;
    POINT win_coord;
    POINT elm_coord;
}MOUSE_ARG;

typedef struct VALUE_CHANGED_ARG{
    int value;
}VALUE_CHANGED_ARG;

typedef struct KEY_ARG{
    SDL_Keysym keysym;
}KEY_ARG;

typedef struct BUTTON_CHECKED_ARG{
    bool checked;
}BUTTON_CHECKED_ARG;

typedef struct RANGE_CHANGED_ARG{
    int new_range_min;
    int new_range_max;
}RANGE_CHANGED_ARG;
...
typedef struct LINE_INDEX_NODE{
    Uint32 first_char;
