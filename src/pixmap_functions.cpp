...
// ------------------------------------------------------ SET BOUND ABSOLUTE
//it means w and h won't be negative
void set_bound_abs(BOUND_RECT* bound)
{
    if(bound->end_x < bound->start_x)
    {
        int aux = bound->end_x;
        bound->end_x = bound->start_x;
        bound->start_x = aux;
    }
    if(bound->end_y < bound->start_y)
    {
        int aux = bound->end_y;
        bound->end_y = bound->start_y;
        bound->start_y = aux;
    }
}
// ------------------------------------------------------ RESET BOUND
...
SOIL_RECT rect_intersection(SOIL_RECT rect_1, SOIL_RECT rect_2)
{
    ...
    BOUND_RECT bound_1 = rect_to_bound(rect_1);
    BOUND_RECT bound_2 = rect_to_bound(rect_2);
   
    return bound_to_rect(bound_intersection(bound_1, bound_2));
}
// ------------------------------------------------------ RECT INTERSECTION
BOUND_RECT bound_intersection(BOUND_RECT bound_1, BOUND_RECT bound_2)
{
    ...
    if(bound_1.start_x < bound_2.start_x)
    {
        if(bound_1.end_x > bound_2.start_x)
        {
            ...
            {
                inter.start_x = bound_2.start_x;
                inter.end_x = bound_2.end_x;
            }else{
                inter.start_x = bound_2.start_x;
                inter.end_x = bound_1.end_x;
            }
        }else{
            return no_inter;
        }
    ...
    if(bound_1.start_y < bound_2.start_y)
    {
        if(bound_1.end_y > bound_2.start_y)
        {
            ...
            {
                inter.start_y = bound_2.start_y;
                inter.end_y = bound_2.end_y;
            }else{
                inter.start_y = bound_2.start_y;
                inter.end_y = bound_1.end_y;
            }
        }else{
            return no_inter;
        }
...
bool line_rect_intersection(SOIL_RECT rect,POINT* start, POINT* end)
{
...
bool adjust_line_to_size(SIZE size,POINT* start, POINT* end)
{
...
// -------------------------------------------------------------- DRAW GRID
void draw_grid(Uint32* pixmap,SIZE px_size, int grid_unit_w, int offset_x, int offset_y)
{
    
}

// -------------------------------------------------------------- DRAW GRID
void draw_grid(Uint32* pixmap,SIZE px_size, int grid_unit_w)
{
