...
//---------------------------------------------- SEARCH STR
int SoilText::search_str(const SoilString& str, int pos, int n, bool match_case, bool match_accent, bool left_to_right)
{
    int res = soil_string.search_str(str, pos, n, match_case, match_accent, left_to_right);
    return res;
}
//---------------------------------------------- SEARCH REPLACE STR
...
bool SoilText::draw_text_rect_px(SOIL_RECT orig_rect, SoilPixmap* dest_pixmap, SOIL_RECT dest_rect, SoilFont* font, int font_size)
{
    ...
    while(y <= coord_ey)
    {
        ...
        if(y == coord_sy){base_font_y += margin_top;font_rect.h -= margin_top;}
        ...
        while(x <= coord_ex)
        {
            ...
            int ind = font->get_char_index(sch);

            font_rect.y = base_font_y;
            font_rect.x += char_size.w * ind;
...
void SoilText::set_cursor_img(SOIL_RECT orig_rect, SoilPixmap* dest_pixmap, POINT text_dest_offset, SoilFont * font, int font_size, int cursor_pos, bool visible)
{
    ...
    if(!is_valid_cursor_pos(soil_string.get_length(),cursor_pos))
    {
        //return;
    }
